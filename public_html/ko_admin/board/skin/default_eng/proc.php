...
if($lecture_id){
	$board = mysql_fetch_array(mysql_query("SELECT * FROM koweb_lecture_config WHERE id='$board_id' LIMIT 1"));
} else {
...
$category_tmp_=$_POST[category_tmp];
$category_tmp = "";
foreach($category_tmp_ as $value){
	$category_tmp .= $value."^";
}
$category_tmps=substr($category_tmp, 0, -1);
...
if($mode == "modify"){
	...
	alert("수정되었습니다.");
	url($return_url[0]."?board_id=$board_id&lecture_id=$lecture_id&amp;mode=list&amp;no=$no&amp;start=$start");
	...
	if(!$_SESSION[is_admin]){
		if($board[is_membership] != "Y"){
			//비밀번호가 없음
			if($password == "" ){
				url("$PHP_SELF?board_id=$board_id&lecture_id=$lecture_id&amp;start=$start&amp;mode=check&amp;return_mode=$mode&amp;no=$no");
				exit;
			}
			$password = hash("sha256", $password);
	...
	alert("삭제되었습니다.");
	url($return_url[0]."?board_id=$board_id&lecture_id=$lecture_id");
