...
if($mode == "modify"){
	...
	//파일업로드
	$modify_row = mysql_fetch_array(mysql_query("SELECT * FROM board_land WHERE no = '$no' LIMIT 1"));
	...
	if(!$_SESSION[is_admin]){
		if($board[is_membership] != "Y"){
			...
			$check_result = mysql_query($check_query);
			$check_row = mysql_num_rows($check_result);
	...
	if($comment_mode == "delete"){
		...
		if(!$_SESSION[is_admin]){
			if($board[is_membership] != "Y"){
				...
				$check_row = mysql_fetch_array(mysql_query("SELECT * FROM board_comment WHERE no = '$comm_no' LIMIT 1"));
		...
		if($comment_result){
			...
			alert("에러가 발생하여 댓글삭제가 실패하였습니다.111");
			url($return_url[0]."?board_id=$board_id&mode=view&no=$ref_board_no");
		}
		...
		//=========== 코멘트의 코멘트 (코멘트추가) asddddddddddddddda33333333333323232323===========//
		...
		//$comment_query = "UPDATE board_comment VALUES('', '$_SESSION[member_id]', '$board_id', '$ref_board_no', '', '', '', '$CI', '$DI', '$comm_name', '$comm_password', '$comm_phone', '$comm_email', '$comm_zip', '$comm_address1', '$comm_address2', ' $comm_title', '$comm_comments', '$comm_file_1', '$comm_file_2' ,'$comm_file_3', '$reg_date' , '$REMOTE_ADDR', '$chapter_title', '$file_1', '$file_2', '$file_3', '$file_4', '$file_5', '$file_6', '$file_7', '$file_8', '$file_9', '$file_10')";
		
		$comment_query = "UPDATE board_comment SET chapter_title='$chapter_title',  ";

											if($file_1){
												$comment_query.= " file_1='$file_1', ";
											}
											if($file_2){
												$comment_query.= " file_2='$file_2', ";
											}
											if($file_3){
												$comment_query.= " file_3='$file_3', ";
											}
											if($file_4){
												$comment_query.= " file_4='$file_4', ";
											}
											if($file_5){
												$comment_query.= " file_5='$file_5', ";
											}
											if($file_6){
												$comment_query.= " file_6='$file_6', ";
											}
											if($file_7){
												$comment_query.= " file_7='$file_7', ";
											}
											if($file_8){
												$comment_query.= " file_8='$file_8', ";
											}
											if($file_9){
												$comment_query.= " file_9='$file_9', ";
											}
											if($file_10){
												$comment_query.= " file_10='$file_10', ";
											}

							

										$comment_query.= " comments='$comm_comments'  WHERE no='$ref_group' ";

		$comment_result = mysql_query($comment_query);



		/*$check_row = mysql_fetch_array(mysql_query("SELECT * FROM board_comment WHERE ref_group = '$ref_group' ORDER BY ref_depth DESC LIMIT 1"));
		...
		if($comment_query){
			...
			alert("에러가 발생하여 댓글등록이 실패하였습니다.2222");
			url($return_url[0]."?board_id=$board_id&mode=view&no=$ref_board_no");
		}
		...
		//=========== 코멘트등록  --aaaaaaaaaaaaaaaa sdfasdfsadf ===========//
		...
		$comment_query = "INSERT INTO board_comment VALUES('', '$_SESSION[member_id]', '$board_id', '$ref_board_no', '', '', '', '$CI', '$DI', '$comm_name', '$comm_password', '$comm_phone', '$comm_email', '$comm_zip', '$comm_address1', '$comm_address2', ' $comm_title', '$comm_comments', '$comm_file_1', '$comm_file_2' ,'$comm_file_3', '$reg_date' , '$REMOTE_ADDR', '$chapter_title', '$file_1', '$file_2', '$file_3', '$file_4', '$file_5', '$file_6', '$file_7', '$file_8', '$file_9', '$file_10' )";
		$comment_result = mysql_query($comment_query);
		$bref = mysql_insert_id();
		...
		if($comment_result){
			...
			alert("에러가 발생하여 댓글등록이 실패하였습니다.333");
			url($return_url[0]."?board_id=$board_id&mode=view&no=$no");
		}
	}
	//=========== 코멘트등록 끝===========//
	...
	$check_ = mysql_fetch_array(mysql_query("SELECT * FROM $board_id WHERE no='$ref_no' ORDER BY depth DESC LIMIT 1"));
	if($check_[depth] != 0){
	...
	$insert_query = "INSERT $board_id SET depth='$depth',
											...
											link='$link'";
	mysql_query($insert_query);
	$ref_no = mysql_insert_id();

	mysql_query("UPDATE $board_id SET ref_no = '$ref_no', ref_group = '$ref_no' WHERE no='$ref_no' LIMIT 1");
