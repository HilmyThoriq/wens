...
if($mode == "write_proc"){
	...
	// 게시판 ID 중복 체크
	$check = mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM koweb_lecture_config WHERE id='$id'"));
...
*/
	...
	$schema = "CREATE TABLE `$id` (
	  ...
	  `category_tmp` varchar(255) DEFAULT '',
	...
	$sort_ = mysql_fetch_array(mysql_query("SELECT * FROM koweb_lecture_config ORDER BY sort DESC LIMIT 1"));
	$sort = $sort_[sort]+1;

	// 게시판 정보 입력

	@mysql_query("INSERT INTO koweb_lecture_config VALUES('', '$id', '$title', '$skin', '$auth_write', '$auth_read', '$auth_reply', '$auth_delete', '$auth_comment', '$use_category', '$category_detail', '$use_comment', '$use_reply', '$always_secret', '$file_count', '$file_limit_size', '$use_address', '$sms_auth', '$spam_auth', '$dept_query', '$is_membership', '$reg_date', '$state', '$sort', '$active')");
	...
	//기존게시판정보 불러오기
	$board_ = mysql_fetch_array(mysql_query("SELECT * FROM koweb_lecture_config WHERE no='$no' LIMIT 1"));
	...
	@mysql_query("UPDATE koweb_lecture_config  SET title='$title', skin='$skin', auth_write='$auth_write', auth_read='$auth_read', auth_reply='$auth_reply', auth_delete='$auth_delete', auth_comment='$auth_comment', use_category='$use_category', category_detail='$category_detail', use_comment='$use_comment', use_reply='$use_reply', always_secret = '$always_secret' , file_count='$file_count', file_limit_size='$file_limit_size', use_address='$use_address', dept_auth = '$dept_query', reg_date='$reg_date', sms_auth='$sms_auth', spam_auth='$spam_auth', state='$state', is_membership = '$is_membership', active='$active' WHERE no='$no'");
	...
	@mysql_query("RENAME TABLE $board_[id] TO $rename_");

	@rename($_SERVER[DOCUMENT_ROOT] . "/upload/" . $board_[id], $_SERVER[DOCUMENT_ROOT] . "/upload/" . $rename_);
	@mysql_query("DELETE FROM koweb_lecture_config WHERE no = '$no' LIMIT 1");
	$alert_txt = "삭제";
}
...
location.href = "<?=$PHP_SELF?>?type=<?=$type?>&core=<?=$core?>&manager_type=lecture";
</script>
